import { Hymn } from './types';

export const hymns2: Hymn[] = [
  {
    id: '51',
    title: { amharic: 'የሱስ ሆይ አትለፍብኝ', english: 'Pass Me Not, O Gentle Savior' },
    author: { amharic: '', english: 'Fanny J. Crosby\nWilliam H. Doane' },
    verses: [
      { type: 'verse', amharic: 'የሱስ ሆይ አትለፍብኝ ፀሎቴን ስማኝ\nሌሎች ፀጋህን ሲያገኙ አትለፍብኝ', english: '' },
      { type: 'verse', amharic: 'ወደ ዘላለም ዙፋንህ እመጣለሁኝ\nይቅርታህን ለመቀበል እርዳኝ አቤቱ', english: '' },
      { type: 'verse', amharic: 'ስለ ደግነትህ ብቻ ፀጋህን ስጠኝ\nየቆሰለ ልቤን ፈውስ በፀጋህ ልዳን', english: '' },
      { type: 'verse', amharic: 'አንተ ነህ የደስታ ምንጭም ህይወት አንተ ነህ\nበሰማይ እና በምድር ሌላ የለኝም', english: '' },
      { type: 'chorus', amharic: 'የሱስ ክርስቶስ ፀሎቴን ስማኝ\nሌሎች ፀጋህን ሲያገኙ አትለፍብኝ', english: '' }
    ],
    tags: ['REVIVAL & INVITATION']
  },
  {
    id: '52',
    title: { amharic: 'ኃይል አለው በደሙ', english: 'There Is Power in the Blood' },
    author: { amharic: '', english: 'LEJ\nLE Jones' },
    verses: [
      { type: 'verse', amharic: 'ከኃጢአት መንፃት ትወዳለህን? ባፈሰሰልህ\nክቡር ደሙ ክፉን ማሸነፍ ምጎትህ ነውን? መልካም ኃይል አለው በደሙ።', english: '' },
      { type: 'verse', amharic: 'ትሁት መሆንን ትወዳለህን? ባፈሰሰልህ\nክቡር ደሙ የሱስን ማሰብ ያስፈልግሃል? መልካም ኃይል አለው በደሙ።', english: '' },
      { type: 'verse', amharic: 'ፍፁም መሆንን ትወዳለህን? ባፈሰሰልህ\nክቡር ደሙ የሱስ ኃጢአትህን ይፍቀልሃል መልካም ኃይል አለው በደሙ።', english: '' },
      { type: 'verse', amharic: 'ጌታን ማገልገል ትመጣለህን ባፈሰሰልህ\nክቡር ደሙ ምስጋና ለአምላክህ በየቀኑ ስጥ መልካም ኃይል አለው በደሙ።', english: '' },
      { type: 'chorus', amharic: 'ክቡር  ኃይል ኃይል\nአለው በደሙ በክቡር በጉ ደም ኃይል\nሃይል አለው ግሩም ፈዋሽ አዳኝ ደም በአከበረው በጉ ደም', english: '' }
    ],
    tags: ['REVIVAL & INVITATION']
  },
  {
    id: '53',
    title: { amharic: 'መጣሁ ወደ መስቀልህ', english: 'I Am Coming to the Cross' },
    author: { amharic: '', english: 'William McDonald\nWilliam G Fischer' },
    verses: [
      { type: 'verse', amharic: 'መጣሁ ወደ መስቀልህ ምስኪን መኝ ደካማም ነኝ\nሁሉን ነገር ትቻለሁ መድኅንን አገኛለሁ', english: '' },
      { type: 'verse', amharic: 'ልቤ ስትሻህ ነበር በ ኃጢያቴ ስቸገር\nየሱስ በፍቅር ጠራኝ ከኃጢአቴ ሊያነፃኝ', english: '' },
      { type: 'verse', amharic: 'በተስፋህም አምናለሁ እርዳኝ ስለሞትህልኝ\nበምድር ጭንቀት ሳለሁ ኦ የሱስ እጄን ያዘኝ', english: '' },
      { type: 'verse', amharic: 'የሱስ ነፍሴም ያድናል በእርሱ መንፃት ሆነልኝ\nእኔ በርሱ ድኛለሁ ለበጉ ክብር ይሁን', english: '' },
      { type: 'chorus', amharic: 'ባንተ አምንብሃለሁ ኦ የቀራንዮ በግ\nአንተን እለምናለሁ አድነኝ የሱስ አድነኝ', english: '' }
    ],
    tags: ['REVIVAL & INVITATION']
  },
  {
    id: '54',
    title: { amharic: 'እኔ የሱስን ለማየት', english: 'Brightly Beams Our Father\'s Mercy' },
    author: { amharic: '', english: 'PP Bliss' },
    verses: [
      { type: 'verse', amharic: 'እኔ የሱስን ለማየት ከቶ አይሆንልኝም\nየኔ ጠባይ ለድካም ነው የሚመራኝ ለጥፋት\nብርሃንን ለሁሉ ፍጥረት አበራህልን ለኛ\nበዚህ ብቻ እመራለሁ በንፅህና ለመግባት።', english: '' },
      { type: 'verse', amharic: 'እንዴት ያለ ህይወት ሆነ ለተያዙ በመርገም\nከመከራ ቀንበር ሁሉ በሞተበት ቀን ወጡ\nበደሙ የገዛቸውን ያጠራቸዋል አሁን\nተቀበል የጌታን ጥሪ ጨለማ ሳይደርስብህ።', english: '' },
      { type: 'verse', amharic: 'ማነው አሁን የሚጠራ እየጮኸ በፍቅር\nወደኔ ኑ ሁላችሁ ፀጋዬን ተቀበሉ\nእርሱ ነው መድኃኒታችን ስለ አለም የሞተው\nስለዚህ መጥራቱን አክብር ለዘላለም ልትድን።', english: '' }
    ],
    tags: ['REVIVAL & INVITATION']
  },
  {
    id: '55',
    title: { amharic: 'ካለህ የበለጠውን ስጥ', english: 'Give of Your Best To The Master' },
    author: { amharic: '', english: 'Howard B Grose\nCharlotte A Barnard' },
    verses: [
      { type: 'verse', amharic: 'ካለህ የበለጠውን ስጥ ወጣትነትህንም\nሁለመናህን አስረክብ ለእውነት ወታደር ሁን\nየሱስም ምሳሌህ ሆኖ ራሱን ለ አምላክ አስገዛ\nአንተም ለጌታ ታማኝ ሁን የበለጠውን ስጠው', english: '' },
      { type: 'verse', amharic: 'ካለህ የበለጠውን ስጥ ልብህ ለጌታ ይሁን\nበአገልግሎትህ ሁሉ ጌታን አስቀድመው\nአምላክ ወዶህ ልጁን ሰጠህ ፅድቁንም አበራልህ\nየአምላክህን ፍቅር አስብ  የበለጠውን ስጠው', english: '' },
      { type: 'verse', amharic: 'ካለህ የበለጠውን ስጥ ወዶናል ይገባዋል\nላንተ ሲል ህይወቱን ሰዋ የሰማይ ክብሩን ተወ\nሃፍረትህንም ወሰደ ፅድቁን ላንተ ሰጠ\nውለታውን ሁሉ አስብ የበለጠውን ስጠው\nFine\nካለህ የበለጠውን ስጥ ወጣትነህንም\nየአምላክን ጦር እቃ ልበስ እውነትን ይዘህ ገስግስ', english: '' }
    ],
    tags: ['REVIVAL & INVITATION']
  },
  {
    id: '56',
    title: { amharic: 'ኃጥአትህ ቢቀላብህ', english: 'Through Your Sins Be as Scarlet' },
    author: { amharic: '', english: 'Fanny. J Crosby\nWilliam H.Doane' },
    verses: [
      { type: 'verse', amharic: 'ኃጢአትህ ቢቀላብህ እንደ በረድ ይነጻል\nኃጢአትህ ቢቀላብህ እንደ በረድ ይነጻል\nግን ቢቀላ ግን ቢቀላ እንደ ደም እንደ ጥጥ ነጻ\nኃጢአትህ ቢቀላብህ ኃጢአትህ ቢቀላብህ እንደ\nበረድ ይነጻል እንደ በረድ ይነጻል', english: '' },
      { type: 'verse', amharic: 'እሁንም ቃሉን ስሙ ወዳምላክ ተመለሱ\nአሁንም ቃሉን ስሙ ወዳምላክ ተመለሱ\nግሩም ፈቃር ነው ግሩም ፈቃር ነው ጌታችን አድምጹ ቃሉን\nአሁንም ቃሉን ስሙ አሁንም ቃሉን ስሙ\nወዳምላክ ተመለሱ ወዳምላክ ተመለሱ', english: '' },
      { type: 'verse', amharic: 'ይምራል ኃጢአታችሁን ሁለተኛ አያስብም\nይምራል ኃጢአታችሁን ሁለተኛ አያስብም\nህዝቤ ሆይ ኑ ህዝቤ ሆይ ኑ ወደኔ ይጠራል አምላክ\nይምራል ኃጢአታችን ይምራል ኃጢአታችን\nሁለተኛ አያስብም ሁለተኛ አያስብም', english: '' }
    ],
    tags: ['REVIVAL & INVITATION']
  },
  {
    id: '57',
    title: { amharic: 'ኮብልላ ነበረች', english: 'He Saved Me By His Blood' },
    author: { amharic: '', english: '' },
    verses: [
      { type: 'verse', amharic: 'ኮብልላ ነበረች ነፍሴ ከእግዚአብሔር መንገድ\nበጣም የበደልኹኝ ኃጢአተኛም ነበርሁ\nግን አዳኜ ከላይ ፍቅር ሰጠኝ ሰላም\nእጁን ዘረጋና አዳነኝ', english: '' },
      { type: 'verse', amharic: 'ተስፋቢስ ነበርኹኝ ሊያድነኝ ሲመጣ\nግን እርሱ ነገረኝ ነጻ እንደ ምወጣ\nከዚያም አነሳና በጸጋው አዳነኝ\nእጁን ዘረጋና አዳነኝ', english: '' },
      { type: 'verse', amharic: 'የማይገለጽ ደስታ ሰላምን ሰጠኸኝ\nጌታ ህይወቴን ላንተ አስረክባለሁኝ\nእንግዲህ እጄን ያዝ ካንተም ጋር አኑረኝ\nእጁን ዘረጋና አዳነኝ።\nደሙን ለእኔ አፍስሶ አዳነኝ ደሙን\nለእኔ አፍስሶ አዳነኝ የማልረባም ብሆን\nአምላኬን የማላውቅ እጁን ዘረጋና አዳነኝ።', english: '' }
    ],
    tags: ['REVIVAL & INVITATION']
  },
  {
    id: '58',
    title: { amharic: 'የሱስ ከፍሎታል', english: 'Jesus Paid It All' },
    author: { amharic: '', english: 'Elvina M. Hall\nJohn T.Grape' },
    verses: [
      { type: 'verse', amharic: 'ሲል ሰማሁት ጌታን አንተ ደካ ልጅ\nንቃ ትጋ ፀልይ አምልጥ ከጥፋት እጅ', english: '' },
      { type: 'verse', amharic: 'ጌታ ሆይ አምናለሁ ኃይልህ ያንተ ብቻ\nድንጋይ ልብ ለውጦ ለምፅ እንደሚያነፃ', english: '' },
      { type: 'verse', amharic: 'አንዳች ፅድቅ የለኝም ተስፋዬ ፀጋህ ነው\nእንድታጠብ እርዳኝ የሱሴ ባንተ ደም', english: '' },
      { type: 'verse', amharic: 'አስገራሚ ፀጋህ ሃብቴ ጌጤ ክብሬ\nየሱስ ለኔ ሞተ ይኸው ነው መዝሙሬ', english: '' },
      { type: 'chorus', amharic: 'የሱስ ከፍሎታል ስለኔ ተሰዶ\nኃጢያቴን አጠበው ነጭ እንደ በረዶ', english: '' }
    ],
    tags: ['REVIVAL & INVITATION']
  },
  {
    id: '59',
    title: { amharic: 'የሕይወት እንጀራ', english: 'Break Thou the Bread Life' },
    author: { amharic: '', english: 'Mary A. Lathbury\nWilliam F Shrrwin' },
    verses: [
      { type: 'verse', amharic: 'የሕይወት እንጀራ ቁረስልኝ\nበባህር ዳር ምግብ እንደቆረስህ\nበተቀደሰው ገፅ እሻህአለሁ\nመንፈሴ ናፍቆሃል ሕያው ቃል ሆይ', english: '' },
      { type: 'verse', amharic: 'እውነቱን ባርክልኝ ለኔ ለኔ\nበገሊላ ምግብ እንደባረክህ\nነፃ እወጣለሁ ከእስራቴ\nሰላም አገኛለሁ በሙላቴ', english: '' },
      { type: 'verse', amharic: 'መንፈስን ላክልኝ አሁን ለኔ\nአይኔን እንዲነካኝ እንዲያሳየኝ\nከቃልህ እውነቱን ግለፅልኝ\nበመጽሐፍህ እንዳይህ አድርገኝ', english: '' },
      { type: 'verse', amharic: 'የሕይወት እንጀራ ነህ ጌታዬ\nየቅዱሱ ቃልህ መዳኛዬ\nበልቼው እንድኖር ካንተ ጋር\nእውነትህን አስታውሰኝ ፈቃር አምላክ', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '60',
    title: { amharic: 'የጸሎት ሥፍራ', english: 'The Beautiful Garden of Prayer' },
    author: { amharic: '', english: 'Eleanor Allen Schroll\nJ H Filmore' },
    verses: [
      { type: 'verse', amharic: 'የሱስ ለፀሎት ይጠብቀናል አስገራሚ ከሆነው ስፍራ\nበየሱስ ብርሃን ይበራልናል ከተዋበው የጸሎት ሥፍራ።', english: '' },
      { type: 'verse', amharic: 'የሱስ ለፀሎት ይጠብቀኛል ችግሬን ሁሉ አቀርባለሁ\nየሚያጽናና ቃልም እሰማለሁ ከተዋበው የጸሎት ሥፍራ።', english: '' },
      { type: 'verse', amharic: 'የሱስ ለፀሎት ይጠብቀሃል ከዚያ ልትገናኘው ይሻል\nተምበርክከህ በረከት ተቀበል ከተዋበው የጸሎት ሥፍራ።', english: '' },
      { type: 'chorus', amharic: 'አስገራሚው ስፍራ የሚጸለይበት የተዋበ የጸሎት ስፍራ\nየሱስ ሲጠብቀኝ በሩን ሊከፍትልኝ ወደሚያምረው የጸሎት ሥፍራ።', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '61',
    title: { amharic: 'ጥግ ሁነኝ', english: 'Nearer, Still Nearer' },
    author: { amharic: '', english: 'Mrs C.H.Morris\nMrs C.H.Morris' },
    verses: [
      { type: 'verse', amharic: 'ጥግ ሁነኝ ጌታ ቅረበኝም ቅዱስ ነህ\nመድኃኒቴ አስጠጋኝ እቀፈኝ ያዘኝ\nበደረትህ የሱስ ጋርደኝ በዚያ ልረፍበት\nየሱስ ጋርደኝ በዚያ ልረፍበት።', english: '' },
      { type: 'verse', amharic: 'ጥግ ሁነኝ ጌታ ምንም የለኝ ምን ልሰዋ\nለጌታዬ የሱስ በ እርኩስ ነገር\nልቤ ሞላ ዕድልን ስጠኝ መንጻት ሊሆነኝ\nዕድልን ስጠኝ መንጻት ሊሆነኝ።', english: '' },
      { type: 'verse', amharic: 'ጥግ ሁነኝ ጌታ ያንተ ልሁን ስንፍናዬን\nበደስታ ልተው ነኝ ሁሉንም ምቾት\nክብረትንም የተሰቀለውን የሱስን ልይ\nየተሰቀለውን የሱስን ልይ።', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '62',
    title: { amharic: 'የተባረከ ሰዓት ነው', english: 'Tis the Blessed Hour of prayer' },
    author: { amharic: '', english: 'Fanny J Crosby\nW.H. Doane' },
    verses: [
      { type: 'verse', amharic: 'የተባረከ ሰዓት ነው የምንጸልይበት\nወዳምላካችን የሱስ የምንቀርብበት\nበሃይማኖት ብንቀርበው እርሱ ያስጠጋናል', english: '' },
      { type: 'verse', amharic: 'የተባረከ ሰዓት ነው ጌታ የሚቀርበን\nበርኅራኄ ፍቅር ልጆቹን ሲሰማ\nሸክማችንን ከጫማው እንድንጥል ሲነግረን', english: '' },
      { type: 'verse', amharic: 'የተባረከ ሰዓት ነው ተስፋ ለቆረጡ\nለየሱስ ችግራቸውን ያቀርባሉ\nበሩኅሩኅ ልቡ ያሳርፋቸዋል', english: '' },
      { type: 'verse', amharic: 'የተባረከ ሰዓት ነው በጌታ ስናምን\nእርሱ ይሰጠናል የምንፈልገውን\nስቃይ ችግርም ያልፋል በሱ ኃይል ስናምን', english: '' },
      { type: 'chorus', amharic: 'እንዴት ጣፋጭ ዕረፍት ነው በዚያ መሆን ውብ ነው።\nየጸሎት ጊዜ የተባረከ\nእንዴት ጣፋጭ ዕረፍት ነው በዚያ መሆን ውብ ነው።', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '63',
    title: { amharic: 'የጸሎት ሰዓት', english: 'Sweet Hour Of Prayer' },
    author: { amharic: '', english: 'William W Walford\nWilliam Bradbury' },
    verses: [
      { type: 'verse', amharic: 'የጸሎት ሰዓት የደስታ ሰዓት ከአለም የምሸሽበት\nአባቴንም የምሻበት ምኞቴን የምነግርበት\nከመከራ እርቃለሁ ጽናቴንም አገኛለሁ\nከፈተናም እድናለሁ በፀሎትም እቀርባለሁ', english: '' },
      { type: 'verse', amharic: 'የጸሎት ሰዓት የደስታ ሰዓት ጸሎቴ የሚሰማበት\nአምላኬን የማገኝበት ነፍሴ የምትረካበት\nወደዙፋኑም ከጠራኝ ጽጋዬን ታመንምካለኝ\nችግሬንም እጥላለሁ እርሱንም እለምናለሁ', english: '' },
      { type: 'verse', amharic: 'የጸሎት ሰዓት የምወደው መጽናናቱን ልካፈለው\nየሰማይን ቤት እስካየው ሩጫዬን እስክጨርሰው\nበማይሞት ስጋ እነሳለሁ ዋጋዬንም እወስዳለሁ\nበእልልታም እውላለሁ ሳመሰግን እኖራለሁ', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '64',
    title: { amharic: 'ፈቃድህን አድርግ', english: 'Have Thine Own Way Lord' },
    author: { amharic: '', english: 'AAP\nGeorge C Stebbins' },
    verses: [
      { type: 'verse', amharic: 'ፈቃድህን አድርግ ኦ ጌታ ሆይ አንተ ሠሪ ነህ\nእኔ ሸክላ አብጀኝ ስራኝ\nእንደምትወድ ስጠብቅህ ሳል በፈቃዴ', english: '' },
      { type: 'verse', amharic: 'ፈቃድህን አድርግ ኦ ጌታ ሆይ መርምረኝ\nእየኝ ጌታ ዛሬ እንደ በረዶ\nአንጻኝ አሁን በፊትህ ስቀርብ ትሁት ልሁን', english: '' },
      { type: 'verse', amharic: 'ፈቃድህን አድርግ ኦ ጌታ ሆይ መርምረኝ\nእና እርዳኝ ጌታ ኃይል እና ችሎት\nያንተ ናቸው ዳሰኝ ፈውሰኝ መድኃኒቴ', english: '' },
      { type: 'verse', amharic: 'ፈቃድህን አድርግ ኦ ጌታ ሆይ መርምረኝ\nግዛ ሰውነቴን በመንፈስ ሙላኝ\nሁሉ እስኪያይ አንተ መኖርህን በህይወቴ', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '65',
    title: { amharic: 'ግሩም ጸጋው', english: 'Amazing Grace' },
    author: { amharic: '', english: 'John Newton\nEarly American Melody' },
    verses: [
      { type: 'verse', amharic: 'ግሩም ጸጋው ጣፋጭ ቅኔ ጎስቋላን የሚያድን\nየጠፋሁ ነበርሁ አገኘኝ እውር ነበርሁ አየሁ', english: '' },
      { type: 'verse', amharic: 'ጸጋውም ጥፋቴም አሳየኝ ፍርሃቴን አራቀው\nባመንሁ ጊዜ ፀጋው እጅግ ክቡር ሆኖ ታየኝ', english: '' },
      { type: 'verse', amharic: 'በብዙ ችግር አልፌ እዚህ ደርሻለሁ\nፀጋው በድል አሳለፈኝ ቤቴም ያደርሰኛል', english: '' },
      { type: 'verse', amharic: 'እንደ ፀሐይ እያበራን  ለዘላለም ስንኖር\nእግዚአብሔርን ለማወደስ ጊዜ አያጥረንም', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '66',
    title: { amharic: 'ጸሎታችንን ስማ', english: 'Hear Our Prayer O Lord' },
    author: { amharic: '', english: 'George Whelpton' },
    verses: [
      { type: 'verse', amharic: 'ጸሎታችንን ስማ ጌታ ሆይ\nጆሮህን አዘንብል ሰላምህን ስጠን አሜን', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '67',
    title: { amharic: 'ወዳንተ አቅርበኝ', english: 'I Am Thine, O Lord' },
    author: { amharic: '', english: '' },
    verses: [
      { type: 'verse', amharic: 'ድምፅህን ሰማሁ ጌታ ያንተ ነኝ\nፍቅርህን የነገረኝ እጄን ዘርግቼ ወደ ጌታዬ\nወደ አንተ እንድቀርብ', english: '' },
      { type: 'verse', amharic: 'ላገልግሎትህ ቀድሰኝ አሁን\nበመለኮታዊ ኃይል በጽኑ ተስፋ አንተን ልመልከት\nፈቃዴም ላንተ ይሁን', english: '' },
      { type: 'verse', amharic: 'ደስታ ይገኛል በዙፋንህ ፊት\nበጸሎት ስምበረከክ ከአምላኬ ጋር ስነጋገር\nወዳጄ ይሆነኛል', english: '' },
      { type: 'verse', amharic: 'ምን ያህል ጥልቅ ነው እውነተኛ ምንጭ\nየሚገኝ ካንተ ፍቅር በመጨረሻ ልናገኘው ነው\nከዚያ ስንደርስ', english: '' },
      { type: 'chorus', amharic: 'ወደ አንተ አቅርበኝ ጌታ ወደመስቀልህ ቦታ ወደ\nአንተ አንተ አቅርበኝ ጌታ ወደ መስቀልህ ቦታ', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '68',
    title: { amharic: 'አምላኬ ወዳንተ', english: 'Nearer My God to Thee' },
    author: { amharic: '', english: 'Sarah F Adams\nLowell Mason' },
    verses: [
      { type: 'verse', amharic: 'አምላኬ ወዳንተ እቀርባለሁ\nሸክሜ እንኳን ቢከብድ ከነ ሸክሜ\nአሁንም መዝሙሬ ወዳንተ መቅረብ ነው\nወዳንተ መቅረብ ነው አምላኬ ሆይ', english: '' },
      { type: 'verse', amharic: 'ሳላስበው ቀኑ ከንቱ ሄደ\nጨለማ ከበበኝ በበረሃ\nበህልም እንኳ ሆኜ አምላኬ ወዳንተ\nአምላኬ ወዳንተ እቀርባለሁ', english: '' },
      { type: 'verse', amharic: 'የሰማይን መንገድ ግለጥልኝ\nበምህረትህ ሁሉን ስደድልኝ\nመላክትህ ያድሙኝ አምላኬ ወዳንተ\nአምላኬ ወዳንተ እቀርባለሁ', english: '' },
      { type: 'verse', amharic: 'በነቃ ሃሳቤ ሳመሰግን\nከኃዘኔ ሥፍራ ቤቴን ልስራ\nጩኸቴን ለማቅረብ አምላኬ ወዳንተ\nአምላኬ ወዳንተ እቀርባለሁ', english: '' },
      { type: 'verse', amharic: 'ከዚህ በደስታ ወደ ሰማይ\nለመሄድ በ እልልታ እበራለሁ\nመዝሙሬን ስዘምር አምላኬ ወዳንተ\nአምላኬ ወዳንተ እቀርባለሁ', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '69',
    title: { amharic: 'በመከራ ጊዜ', english: 'In the Hour of Trial' },
    author: { amharic: '', english: 'James Montgomery\nSpencer Lane' },
    verses: [
      { type: 'verse', amharic: 'በመከራ ጊዜ የሱስ አስበኝ\nበክህደቴም ምክንያት እንዳትለየኝ\nእምነቴ መድከሙ በመልኬ ሲታይ\nአትተወኝ ጌታ ካንት እንዳልለይ', english: '' },
      { type: 'verse', amharic: 'በማይጠቅም ደስታ ዓለም ያታልላል\nበተንኮልም ወጥመድ ሰውን ያጠምዳል\nአስታውሰኝ ያን ጊዜ ጌተሰማኔን\nደግሞም ቀራንዮን የሾህ አክሊልህን', english: '' },
      { type: 'verse', amharic: 'ሐዘንን ችግርን በምህረት ብትሰድ\nህመም ቢደርስብኝ ወዳንተ ስሄድ\nአደራ ጌታ ሆይ እጅህ ይታየኝ\nችግሬን ስነግርህ አምላክ ደግፈኝ', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '70',
    title: { amharic: 'ነፍሴን ተናገር', english: 'Speak to My Soul' },
    author: { amharic: '', english: '' },
    verses: [
      { type: 'verse', amharic: 'ነፍሴን ተናገር የሱስ ቅርስ ብለህ ንገረኝ\nበቸርነት አስታውቀኝ አልተውህም በለኝ\nእንዳዳምጥህ ስጠኝ ቃልህን እንድሰማ\nነፍሴ ምስጋና ትምላ ባንተ ደስ ይበለኝ', english: '' },
      { type: 'verse', amharic: 'ልጆችህን ተናገር መንገድህን ምራቸው\nበደስታም ምላቸው ጸሎት አስተምራቸው\nህይወታቸውን ቀድስ ላንተ ሁሉን ይስጡህ\nመንግስትህን ያፋጥኑ አንተን እስክናይህ', english: '' },
      { type: 'verse', amharic: 'እንደ ጥንቱ ተናገር ምኞትህ ይገለፅ\nተግባሬን ሁሉ ልወቅ ፈቃድህን ልፈፅም\nምራኝ አከብርህ ዘምድም ክብርህን እንዳሳይ\nበደስታ እንድታዘዝ ሁሌም እንዳከብር', english: '' },
      { type: 'chorus', amharic: 'በለዘብታ ንገረኝ በፍቅር ንገረኝ\nታሸንፋለህ በለኝ ነፃም ትወጣለህ\nበየቀኑ ንገረኝ ሁልጊዜ አስታውሰኝ\nአሁን ቃልህን ልስማ አልተውህም ስትል', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '71',
    title: { amharic: 'የሱስ ቅዱስ እጅህ', english: 'Now Spread Thy Wings of Love' },
    author: { amharic: '', english: '' },
    verses: [
      { type: 'verse', amharic: 'ኦ የሱስ ቅዱስ እጅህ በኔ ላይ ይስፈር\nበሃዘንም በደስታም ከአንተ ዘንድ ልረፍ\nጥበቤም ኃይሌም ጥጌም ሁሉም በ ሁሉ ይሁን\nእኔም በቀን በሌትም ፀጋ ውስጥ ልሁን', english: '' },
      { type: 'verse', amharic: 'ስጠኝ ያንተን ይቅርታ በደምህም አንፃኝ\nአግጋልኝ ቅዱስ ሃሳብ ፈቃድህም ይምራኝ\nሁላችንንም ጠብቅ መለክ በዚህ ይስፈር\nአሁንም በሰላምህ የሱስ እንደር', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '72',
    title: { amharic: 'ኦ ጌታ ሆይ ምህረትህ', english: 'O Lord In Your Mercy' },
    author: { amharic: '', english: 'Tebdje Gudaye\nEarly American Melody' },
    verses: [
      { type: 'verse', amharic: 'ኦ ጌታ ሆይ በምህረትህ ወዳንተ አቅርበን\nበስራችህ የጠፋን ነን በፀጋህ አድነን', english: '' },
      { type: 'verse', amharic: 'ኃጢአታችህ ይቅር በለን አሁን ተቀበለን\nበዚህ ዓለም አትተወን ወዳንተ ውሰደን', english: '' },
      { type: 'verse', amharic: 'ኦ መንፈስ ቅዱስ መለኮት ና ና ውረድልን\nበኃይልህም አነቃቃን ለፅድቅህም አብቃን', english: '' },
      { type: 'verse', amharic: 'የአምላክ ልጅ ሃሌሉያ በደም የዋጀኸን\nዛሬም ደግሞ አትለየን ለዘላለም አሜን።', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '73',
    title: { amharic: 'የምሰጥህ ሁሉ', english: 'We Give Thee But Thine Own' },
    author: { amharic: '', english: 'William Walsham How.c\nFrom Cantica Laudis' },
    verses: [
        { type: 'verse', amharic: 'የምንሰጥህ ሁሉ ካንተ የመጣ ነው\nያለንም ሁሉ ያንተ ነው አደራ ያልከን ነው።', english: ''}
    ],
    tags: ['PRAYER']
  },
  {
    id: '74',
    title: { amharic: 'በመንፈስህ ሙላኝ', english: 'With Thy Spirit Fill Me' },
    author: { amharic: '', english: '' },
    verses: [
        { type: 'verse', amharic: 'በመንፈስህ ሙላኝ በመንፈስህ ሙላኝ\nሁሉን ላንት ልስጥ እርዳኝ በመንፈስህ ሙላኝ', english: ''}
    ],
    tags: ['PRAYER']
  },
  {
    id: '75',
    title: { amharic: 'አባታችን ጌታችን ሆይ', english: 'Majestic Sweetness Sits Enthroned' },
    author: { amharic: '', english: 'Samuel Sttennett\nThomas Hasting' },
    verses: [
      { type: 'verse', amharic: 'አባታችህ ጌታችን ሆይ የ\nምትኖር በሰማይ መንግስትህ ትምጣልን ከላይ\nክብርህን እንድናይ ክብርህን እንድናይ', english: '' },
      { type: 'verse', amharic: 'ፈቃድህ በሰማይ ቀድሞ ህይ\nወት  እንደጎነች እንዲሁም በምድር ደግሞ\nሰላምህን ስጠን ሰላምህን ስጠን', english: '' },
      { type: 'verse', amharic: 'ዛሬ ለህዝብህ ስጠን የዕ\nለት ሲሳያችን በደላችንንም ሁሉ\nይቅር  እንድትለን ይቅር  እንድትለን', english: '' },
      { type: 'verse', amharic: 'ጌታሆይ አታግባን አሁን ከ\nክፉ ፈተና አምላክ አንተ ካልረዳኸን\nኃይል የለንምና ኃይል የለንምና', english: '' },
      { type: 'verse', amharic: 'ኃይልና ምስጋና ክብርም መን\nግሥትም ያንተ ናት አሜን እስከ ዘለዓለም\nይሁንልን እረፍት ይሁንልን ዕረፍት።', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '76',
    title: { amharic: 'ጌታ ጽዋዬን ሙላ', english: 'Fill My Cup Lord' },
    author: { amharic: '', english: 'R.B\nRichard Blanchard' },
    verses: [
      { type: 'verse', amharic: 'በጉድጓዱ አጠገብ እንደዚያች ሴት እኔም ስፈልግ ነበርኩኝ\nኋላም አዳኜ ሲለኝ ሰማሁ ከኔ ቅዳ ከቶ አትጠማም', english: '' },
      { type: 'verse', amharic: 'በዚህ ዓለም ብዙዎች ያን ሲመኙ ዓለም ለምትሰጣቸው\nግን ከሁሉ የሚበልጠውን ጌታዬ ለኔ ይሰጠኛል', english: '' },
      { type: 'verse', amharic: 'ወንድሜ ሆይ የ ዓለም ደስታ ጥማትህን አያረካም\nስለዚህ ጌታ ያረካሃል ዳግመኛ ከቶ ላትጠማ', english: '' },
      { type: 'chorus', amharic: 'የኔን ፅዋ እንካ ጌታዬ ና ውሰድና ነፍሴን አርካት\nየሰማይን እንጀራ ቆርሰህ ስጠኝ ፅዋዬን ሞልተህ አድነኝ ጌታ', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '77',
    title: { amharic: 'የምህረት አምላክ', english: 'God of Blessing' },
    author: { amharic: '', english: '' },
    verses: [
      { type: 'verse', amharic: 'የበረከት አምላክ የፍቅር አምላክ\nየሰላም ንጉስ የማትለወጠው\nየሱስ ሆይ ናልን ወደኛ።', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '78',
    title: { amharic: 'እንድቀደስ እርዳኝ', english: 'Take Time to be Holy' },
    author: { amharic: '', english: 'William D Long Staff\nGeorge C Stebbins' },
    verses: [
      { type: 'verse', amharic: 'እንድቀደስ እርዳኝ የብርሃን አባት\nኃጢአቴ ከብዶኛል ከፊትህ ወደቅሁ\nሕሊናው ያደፈ ሊያይህ አይችልም\nአንተ ካለህበት እንድገኝ እርዳኝ', english: '' },
      { type: 'verse', amharic: 'እንድቀደስ እርዳኝ ብሩክ አዳኜ\nኦ አትዘግይብኝ ለውጠኝ ጌታ\nጠባይህን አግባልኝ ሰይጣንን አውጣ\nበክብር ተገለጥ ጨለማን አጥፋ', english: '' },
      { type: 'verse', amharic: 'እንድቀደስ እርዳኝ ኦ ቅዱስ መንፈስ\nና ና ቀድስልኝ የ አንተን መቅደስ\nጣዖትን አስወግድ ዙፋንህ ይምጣ\nአንተ ብቻ ግዛ ስልጣን ያንተ ነው', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '79',
    title: { amharic: 'ጌታ ሆይ ክርስቲያን ልሁን', english: 'Lord I Want to Be a Christian' },
    author: { amharic: '', english: 'American Negro spiritual' },
    verses: [
      { type: 'verse', amharic: 'ጌታ ሆይ ክርስቲያን ልሁን\nጌታ ሆይ ክርስቲያን ልሁን በልቤ ውስጥ በልቤ ውስጥ\nጌታ ሆይ ክርስቲያን ልሁን', english: '' },
      { type: 'verse', amharic: 'ጌታ ሆይ ይበልጥ ልቀደስ\nጌታ ሆይ ይበልጥ ልቀደስ\nጌታ ሆይ ይበልጥ ልቀደስ', english: '' },
      { type: 'verse', amharic: 'ጌታ ሆይ አፍቃሪ ልሁን\nጌታ ሆይ አፍቃሪ ልሁን\nጌታ ሆይ አፍቃሪ ልሁን', english: '' },
      { type: 'verse', amharic: 'እንደ ይሁዳም እንዳልሆን\nእንደ ይሁዳም እንዳልሆን\nእንደ ይሁዳም እንዳልሆን', english: '' },
      { type: 'verse', amharic: 'አምላክ ሆይ የሱስን ልምሰል\nአምላክ ሆይ የሱስን ልምሰል\nአምላክ ሆይ የሱስን ልምሰል', english: '' },
      { type: 'chorus', amharic: 'በልቤ ውስጥ በልቤ ውስጥ በልቤ ውስጥ በልቤ ውስጥ', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '80',
    title: { amharic: 'ወደ የሱስ ተመልከት', english: 'Turn Your Eyes Upon Jesus' },
    author: { amharic: '', english: 'H.H.L\nH H. Lemmel' },
    verses: [
      { type: 'verse', amharic: 'ወደ የሱስ ተመልከት ግሩም አስደናቂ ፊቱን የዚህ ዓለም ሐብት ሁሉ\nጠፊ ነው ሁሉም ያልፋል ግርማውን ስናይ።', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '81',
    title: { amharic: 'እኔስ ጌታ ሆይ', english: 'O! Lord I Am Freed' },
    author: { amharic: '', english: '' },
    verses: [
      { type: 'verse', amharic: 'ቃልህን ለማንበብ ዓይኔ ታወረ እንዳልጸልይ ጉልበቴ ታሠረ\nአንተን እንዳላስብ አእምሮዬ ዛለ እግሬም ወደ ጥፋት ኮበለለ\nእኔስ ጌታ ሆይ ድካም ይዞኛል ድምጼንም ማሰማት አቅቶኛል\nወድቄአለሁኝ ጌታ ሆይ አንሳኝ እባክህ ሕይወቴን ለውጥልኝ።', english: '' },
      { type: 'verse', amharic: 'ጠፋሁኝ ጌታ ካልደረስክልኝ የጽድቅህን መንገድ ካልመራኸኝ\nበኃጢአት ባሕር ልሰጥም ነውና ጌታ ሆይ እባክህ እጄን ያዘኝ\nአትጨክንም የሱስ በኔ በልጅህ ታውቃለህና ጉድለቴን\nጌታ ሆይ አሁን ኃጢአቴን ማረኝ የምመራበትን ብርሃን ስጠኝ።', english: '' },
      { type: 'verse', amharic: 'አየሁ ጌታዬ ወዲህ ሲመጣ እኔኑ ከጥፋት ሊያወጣ ልጁን ላከልኝ\nከሞት አዳነኝ ከእንግዲህስ ከጌታዬ ጋር ነኝ\nቀረት አልልም እርሱ ሲመራኝ እኔኑ ሊወስድ ስለሚያስብ\nመስቀሉ ለኔ ቢከብደኝ እንኳን እርሱን ለመሸከም አልቦዝንም።', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '82',
    title: { amharic: 'የሱስ ልንገር', english: 'I Must Tell Jesus' },
    author: { amharic: '', english: 'Ehsha A Hoffman' },
    verses: [
      { type: 'verse', amharic: 'ለየሱስ ልንገር ስቃዬን ሁሉ ሸክሙ ብቻዬን\nአቅቶኛል በጭንቀቴ ጌታ ይረዳኛል\nይወደኛል ለኔም ያስባል።', english: '' },
      { type: 'verse', amharic: 'ለየሱስ ልንገር ችግሬን ሁሉ የሚራራልኝ\nወዳጄ ነው ኦ ከለመንኩት ይታደገኛል\nችግሬን ቶሎ ያስወግዳል።', english: '' },
      { type: 'verse', amharic: 'ኦ የዓለም ክፋት እንዴት ያታላል ኦ ልቤስ እንዴት\nይፈተናል ለየሱስ ልንገር እሱ እንዲረዳኝ\nድል እንዳደርጋት يቺን ዓለም።', english: '' },
      { type: 'chorus', amharic: 'ለየሱስ ልንገር ለየሱስ ልንገር ብቻዬን አልቻልኩ ልሸከመው\nለየሱስ ልንገር ለየሱስ ልንገር የሱስ ብቻ ነው የሚረዳኝ።', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '83',
    title: { amharic: 'በልቤ ውስጥ', english: 'Into My Heart' },
    author: { amharic: '', english: 'H D C\nHarv D Clarke' },
    verses: [
      { type: 'verse', amharic: 'በልቤ ውስጥ በልቤ ውስጥ አንተ ግባበት ኦ የሱስ\nግባ ዛሬ ከኔ ጋር ሁን በልቤ ግባ ኦ የሱስ።', english: '' },
      { type: 'verse', amharic: 'በልቤ ውስጥ በልቤ ውስጥ ብርሃንህ ይብራ ኦ የሱስ\nዛሬም ነገም ለዘላለም በልቤ አብራ ኦ የሱስ።', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '84',
    title: { amharic: 'ጌታ ሆይ ስንጸልይ', english: 'As We Come to You in Prayer' },
    author: { amharic: '', english: 'Ralph Carmichael\nIrregular' },
    verses: [
      { type: 'verse', amharic: 'ጌታ ሆይ ስንጸልይ ሐሳባችንን አርቀው ከከበበን የዓለም ችግር ወዳንተ ፀጋ አቅርበን\nበፍቅርህ ሕይወታችን ለውጥ ነፍሳችንን ከላይ አርካልን በዚህ\nሰዓት ያንተ ልጆች ሁሉ በፀሎት በዙፋንህ ፊት ይምጡ አሜን።', english: '' }
    ],
    tags: ['PRAYER']
  },
  {
    id: '85',
    title: { amharic: 'ልረፍ በየሱስ ክንዶች', english: 'Safe in the Arms of Jesus' },
    author: { amharic: '', english: 'Fanny J Crosby\nW H Doane' },
    verses: [
      { type: 'verse', amharic: 'ልረፍ በየሱስ ክንዶች ልረፍ በእቅፉ በፍቅሩ\nከከለለኝ ነፍሴ አትፈራም ከቶ ስማ ከሰማይ መዝሙር\nበመላእክት ሲዘመር በክብር የሚዘመር በብርጭቆ ባሕር።', english: '' },
      { type: 'verse', amharic: 'ልረፍ በየሱስ ክንዶች ልረፍ ከሰቀቀን ልረፍ ካ\nለም ፈተና ከኃጢዓትም ጭምር ከኃዘን ነፃ ልንሆን\nደግሞም ከጥርጣሬ ለጥቂት ጊዜ ብቻ እንሰቃያለን።', english: '' },
      { type: 'verse', amharic: 'የሱስ የነፍሴ አምባ ስለኔ ሞተሃል እምነቴ\nበዚህ አምባ ለዘላለም ጸንቷል ስለዚህም ልታገሥ\nጨለማ እስኪፈፀም የምታምረውም ንጋት እስክትመጣ ድረስ።', english: '' },
      { type: 'chorus', amharic: 'ልረፍ በየሱስ ክንዶች ልረፍ በእቅፉ በፍቅሩ ከከለለኝ ነፍሴ አትፈራም ከቶ።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '86',
    title: { amharic: 'የሱስ የልቤ ወዳጅ', english: 'Jesus Lover of My Soul' },
    author: { amharic: '', english: 'Charles Welsey\nSimeon B Marsh' },
    verses: [
      { type: 'verse', amharic: 'የሱስ የልቤ ወዳጅ ወደ አንተ አስጠጋኝ\nመከራ ሲመጣብኝ ሞገዱ ሲነሣብኝ\nሸሽገኝ ልዑል አምላክ የሕይወት ችግር እስኪያልፍ\nወደ ሰማይ አድርሰኝ መንገዴን ሁሉ ምራኝ።', english: '' },
      { type: 'verse', amharic: 'ያላንተ ጥግ የለኝም በአንተ ታምኛለሁ\nብቻዬን አትተወኝ አጽናናኝ ተሸከመኝ\nእታመንብህአለሁ እርዳታህን እሻለሁ\nደካማ ሰውነቴን በፍቅርህ እኔን ጋርደኝ።', english: '' },
      { type: 'verse', amharic: 'ባንተ ብቻ ከታመንሁ ሁሉንም አገኛለሁ\nደግፍ የደከሙትን ፈውስ የታመሙትን\nአንተ እውነተኛ ነህ እኔ ኝ ያልነጻሁ ነኝ\nበኃጢዓት የተሞላሁ አንተ ታነጻኛለህ።', english: '' },
      { type: 'verse', amharic: 'በፀጋ የተሞላህ ለኃጢዓት ይቅርታ ሰጭ\nኃጢአቴን አስወግደህ ና በንጽህና ያዘኝ\nአንተ የሕይወት ምንጭ ነህ በነጻ ልቀበልህ\nበልቤ ውስጥም ፍለቅ ለዘላለም ኑርልኝ።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '87',
    title: { amharic: 'ከየሱስ ጋር በደኅና እሔዳለሁ', english: 'Any Where with Jesus' },
    author: { amharic: '', english: 'Jessie H Brown\nD B Towner' },
    verses: [
      { type: 'verse', amharic: 'ከየሱስ ጋር በደኅና እሔዳለሁ በዚህ ዓለም\nላይ በርሱ ከተመራሁ ያለርሱ ደስታዬ ከንቱ\nይሆናል እርሱ ግን ፍርሃትን ሁሉ ያጠፋል።', english: '' },
      { type: 'verse', amharic: 'የሱስ ክርስቶስ ከኔ ከቶ አይርቅም ሌላ ወዳጅ\nሲረሣ አይረሣኝም እጆቹ በጭንቀት መንገድ\nቢመሩኝ የሱስ ከኔ ጋር ስላለ ይመስገን።', english: '' },
      { type: 'verse', amharic: 'ከየሱስ ጋር በሰላም አርፋለሁ በሌት ጨ\nለማ ስከበብ ሣለሁ በደስታ እንደምነቃም\nአውቃለሁ በሚያምረውም ቤት አብሬ እኖራለሁ።', english: '' },
      { type: 'chorus', amharic: 'ከማንም ከምንም ከቶ አልፈራም ከየሱስ በደኅና እሔዳለሁ።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '88',
    title: { amharic: 'ትጉ አድርገኝ', english: 'More Dilligence' },
    author: { amharic: '', english: '' },
    verses: [
      { type: 'verse', amharic: 'ጊዜ እንዳይጠፋ ትጉ አድርገኝ\nኃጢዓተኛ አይሙት ተስፋ ሣያገኝ\nእንዴት ሥራ ልፍታ ክርስቶስን ያወቅሁ\nፍቅርህን እንድነግር ትጋትን ስጠኝ።', english: '' },
      { type: 'verse', amharic: 'ለጠፉት በጎችህ ጠባቂ አርገኝ\nእነሱን በማሰብ ፈልጌ ላገኝ\nባጥር በጎዳና ባገር በባሕር\nለጠፉቱ ኃጥዓን አድርገኝ መምሕር።', english: '' },
      { type: 'verse', amharic: 'የፍቅር ምስጋና ላቅርብ ለጌታ\nስጦታንም ደግሞ ለፈጣሪዬ\nስለጸጋውም ሁሉ ጸሎትን ላቅርብ\nበሥራዬ ሁሉ እርሱን ደስ ላሰኝ።', english: '' },
      { type: 'verse', amharic: 'ንጽህናን ስጠኝ ኃጢዓትን ልጥላ\nውስጡ ሰውነቴ ደግነት ይምላ\nራሴን ከመውደድ ነፃ አድርገኝ አባት ሆይ\nአሁን መንፈስህን ስጠኝ የሱስን ልምሰል።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '89',
    title: { amharic: 'ኦ ከእግዚአብሔር ጋር ልራመድ', english: 'O Let me Walk with Thee' },
    author: { amharic: '', english: 'Mrs L D Avery Stuttle\nEdwin Barnes' },
    verses: [
      { type: 'verse', amharic: 'ኦ ከእግዚአብሔር ጋር ልራመድ ኄኖክ በሔደበት መንገድ\nደካማ እጄን ይዘህ ምራኝ በጣፋጭ ድምፅህ ምከረኝ\nመንገዱ እንዳይጠፋብኝ መድኅን የሱስ አትለየኝ።', english: '' },
      { type: 'verse', amharic: 'አልችልም ብቻዬን ልሔድ ሲንዣበብ የሰማይ ሞገድ\nተጠምደው ሳል ሺህ ወጥመዶች ከበውኝ ሳል ሺህ ጠላቶች\nፀጥ አርግና ይህን ሞገድ ኦ ጌታ ሆይ አብረን እንሒድ።', english: '' },
      { type: 'verse', amharic: 'እጄን በእጅህ ላይ አድርጌ የምድርንም ተድላ ንቄ\nበድፍረት እራመዳለሁ መስቀልህን እሸከማለሁ\nወደ ጽዮን በር እስክደርስ ጌታ ሆይ አብረን እንጓዝ።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '90',
    title: { amharic: 'የሱስን ትቼ ዓለምን ባተርፍ', english: 'If I Gained the World' },
    author: { amharic: '', english: '' },
    verses: [
      { type: 'verse', amharic: 'የሱስን ትቼ ዓለምን ባተርፍ የእለት ሕይወቴ ጥቅመ ቢስ ነው\nበሃብቴ ብዛት እረፍት የለኝም ልቤም ሰላምን አያገኝም።\nየሱሴን ትቼ ዓለምን ባተርፍ ጥረቴ ሁሉ አይረባኝም\nይልቅ የሱስን የኔ ባደርገው ለልቤ ሰላም አገኛለሁ።', english: '' },
      { type: 'verse', amharic: 'ሰዎች ቢያፈቅሩኝ ሐብቴም ቢበዛ ስሜንም ደግሞ ቢያከብሩልኝ\nተስፋም ባይኖረኝ ልሠራ ወደብ ተጉዤ ሄጄ የት ላርፍ ነኝ፣\nየሱሴን ብክድ የሞተልኝን መስቀሉን ብጥል ያዝ ያለኝን ጌታ\nሲገለጽ ባሰቃቂው ቀን ማንም አይችልም እኔን ሊያድን።', english: '' },
      { type: 'verse', amharic: 'አልችልም እኔ ልኖር ብቻዬን ያለየሱስ በዚች ዓለም\nመንገዴ ያለ እርሱ ይጨልማል ልቤም በሐዘን ይጨነቃል።\nብኖርስ እንኳ ያለ መድህኔ በጠራኝ ጊዜ ምን ይዋጠኝ\nበሌላ መንገድ ልሄድ ብሞክር ሕይወት የለኝም ለዘላለም።', english: '' },
      { type: 'verse', amharic: 'የየሱስ መሆን ታላቅ ደስታ ነው ልብ የሚፈውስ ቸር አዳኝ ነው\nኃጢአቴን ሁሉ ይቅር ይለኛል ልቤን በደሙ ያጥብልኛል።\nየሱስን ብይዝ የሱሴን ብቻ ምንም ባልፈልግ\nከርሱ ሌላ የሚያስፈልገኝ ይሰጠኛል ለዘላለም ይጠብቀኛል።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '91',
    title: { amharic: 'ወደ ጽዮን እንጓዝ', english: 'We Are Marching to Zion' },
    author: { amharic: '', english: 'Isaac Watts\nRobert Lowry' },
    verses: [
      { type: 'verse', amharic: 'ያምላክ ወዳጆች ኑ ደስታችሁ ትታወቅ\nኑ ባንድነት እንዘምር ላምላካችን ቅዱስ መዝሙር\nየርሱን ዙፋን ከበን እናንግሠው በክብር።', english: '' },
      { type: 'verse', amharic: 'እናንተ እምቢተኞች ተነሡ ዘምሩ\nየኛን አምላክ የማታውቁ ኑ ከርሱ ጋራ ታረቁ\nየጌታ ልጆች ግን ደስታችሁ ትታወቅ።', english: '' },
      { type: 'verse', amharic: 'እንግዲህ እንዘምር እንባችንም ይድረቅ\nባማኑኤል መንገድ ላይ ነን ድንገት ፈርተን እንዳንወድቅ\nስንጓዝ ወደርሱ ወዳማረው አገር።', english: '' },
      { type: 'chorus', amharic: 'ወደ ጽዮን እንጓዝ ወደዚያች ኢየሩሳሌም\nእንገስግስ ወደ ጽዮን ወዳምላካችን ከተማ።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '92',
    title: { amharic: 'መድኃኒቴን አልለይም', english: 'The Savior with Me' },
    author: { amharic: '', english: 'Lizzie Edwards\nJohn R Sweney' },
    verses: [
      { type: 'verse', amharic: 'መድኃኒቴን አልለይም መንገዱ እንዳይጠፋኝ\nከፊቱ ዘንድም አልርቅም በክንዶቹ እንዲአቅፈኝ።', english: '' },
      { type: 'verse', amharic: 'መድኃኒቴን አልለይም እምነቴ እንዳይደክም\nበቃሉ ያበረታኛል ሌሎች በማይሰጡኝ ቃል።', english: '' },
      { type: 'verse', amharic: 'መድኃኒቴን አልለይም በሕይወቴ ጎዳና\nብደሰትም ቢከፋኝም በችግር በመከራ።', english: '' },
      { type: 'verse', amharic: 'መድኃኒቴን አልለይም ዓይኖቹ እንዲመሩኝ\nወደ ዮርዳኖስ ሊያደርሰኝ ወዲያ ማዶ ሊያሻግረኝ።', english: '' },
      { type: 'chorus', amharic: 'አሁን ነፍሴ አትፈራም የሱሴ ይመራኛል በደስታም እታዘዛለሁ እርሱን እከተላለሁ።\nአሁን ነፍሴ አትፈራም አትፈራም የሱሴ ይመራኛል ይመራኛል', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '93',
    title: { amharic: 'ስትጨነቅ ሳለህ የሱስን ጥራ', english: 'Call to Jesus in Trouble' },
    author: { amharic: '', english: 'Raskt ock beslutsamt\nSvensk folkmelodi' },
    verses: [
      { type: 'verse', amharic: 'ስትጨነቅ ሳለህ ጥራ የሱስን ከጥፋት ሁሉ ለመዳን\nያለህን ድካም ግለጽለት አሁን ችግርህን ሁሉ ንገረው\nበአዳኝ ፀጋው ያጠነክርሃል ግን ባንተ ሥራ ፈጽመህ ሞተሃል\nስለዚህ ቶሎ ወደ የሱስ ቸኩል የፀጋ ኃይሉን ተቀበል።', english: '' },
      { type: 'verse', amharic: 'አምላካችን ሀዘንን ሁሉ ያያል የልብን ጩኸት ይሰማል\nየሚያድስ ኃይልም ይሰድልሃል በእምነት ብቻ ተቀበል\nአንተን ይወዳል በፍቅሩም ይጠራል የነፍስን ጥፋት ከቶ አልወደደም\nስለዚህ የሱስን ሊሰዋ ላከው በሞቱም ሁሉን አዳነ።', english: '' },
      { type: 'verse', amharic: 'ባጭሩ ዘመን በእግድነት ኑሮህ ጨለማ ብቻ የሞላ\nወደ ላይ አሻቅበህ የሱስን እይ ጨለማውንም ያጠፋል\nተስፋ አድርገው ለሁሉ ዘመንህ እርሱ ኃይልህ ነው\nከሞትህ ያዳነህ ብርሃኑ ይሠራል በልብህ ገብቶ ዕውርነትህን ያበራል።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '94',
    title: { amharic: 'አቀበቱን እወጣለሁ', english: 'I am Pressing on the Upward Way' },
    author: { amharic: '', english: 'Johnson Oatman Jr\nCharles H Gabriel' },
    verses: [
      { type: 'verse', amharic: 'አቀበቱን እወጣለሁ በየቀኑም አዘግማለሁ\nእየጸለይሁ እጓዛለሁ አምላክ ሆይ አዝልቀኝ እያልሁ።', english: '' },
      { type: 'verse', amharic: 'ፍርሃትና ጥርጥርም ካለበት ቦታ አልኖርም\nሌሎች ቢኖሩ ያን ወደው የኔ ምኞት በላይ ይሁን።', english: '' },
      { type: 'verse', amharic: 'ዓለምን አሸንፋለሁ ከሰይጣንም አመልጣለሁ\nበሃይማኖት ይሰማኛል መላእክት ሲዘምሩ ሣል።', english: '' },
      { type: 'verse', amharic: 'እላይ መድረስ እሻለሁ የክብሩን ብርሃን አያለሁ\nግን ከዚያ ላይ እስከምደርስ አምላክ ከኔ አትመለስ።', english: '' },
      { type: 'chorus', amharic: 'አምላኬ ሆይ ና አቁመኝ በሰማይ ሜዳ አድርሰኝ\nአምላኬ ደህና አዝልቀኝ ከላይኛው ሜዳ አድርሰኝ።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '95',
    title: { amharic: 'የሱስን ማገልገል', english: 'It Pays to Serve Jesus' },
    author: { amharic: '', english: 'Frank C Huston' },
    verses: [
      { type: 'verse', amharic: 'የሱስን ማገልገል ዋጋ ይሰጣል ፍጹም ደስታ ከርሱ ይገኛል\nበቃሉ መታመን ታላቅ ዕረፍት ነው ስለዚህ ና አገልግለው።', english: '' },
      { type: 'verse', amharic: 'የሱስን አገልግል የሆነ ቢሆን በሥራህ ሁሉ ታማኝ ሁነህ ቁም\nበቃሉ መታመን ታላቅ ኃብት አለው ስለዚህ ና አገልግለው።', english: '' },
      { type: 'verse', amharic: 'ምናልባት ጨለማ ይሆንብናል የሚፈትነን ኃዘን ይመጣል\nክቡር አዳኛችን ጠባቂያችን ነው ስለዚህ ና አገልግለው።', english: '' },
      { type: 'chorus', amharic: 'የሱስን አገልግል ዕድሜህን ሁሉ ዋጋ ታገኛለህና\nበመንገድህ እንኳ ችግር ቢያገኝህ እርሱ ጋሻ ይሆንልሃል።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '96',
    title: { amharic: 'ይመራኛል', english: 'He leadeth Me' },
    author: { amharic: '', english: 'J H Gilmore\nWilliam B Bradbury' },
    verses: [
      { type: 'verse', amharic: 'ይመራኛል ይመራኛል በቃሉ ኃይል ያጽናናኛል\nበሥራዬ በኑሮዬ ምራኝ በጅህ ኦ ጌታዬ።', english: '' },
      { type: 'verse', amharic: 'በጨለማ ቦታ ብሔድ በኤደን ገነት ብራመድ\nበጸጥታ በሞገድም አምላኬ ከኔ አይለይም።', english: '' },
      { type: 'verse', amharic: 'ሥራዬ እዚህ ከፈጸምሁ በፀጋህ ድልን ካገኘሁ\nአልፈራም ሞት ቢመጣብኝ ዮርዳኖስን ካሻገርኸኝ።', english: '' },
      { type: 'chorus', amharic: 'ይመራኛል ይመራኛል በገዛ እጁ ይመራኛል\nተከታዩ እሆናለሁ በርሱ እጅ ከተመራሁ።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '97',
    title: { amharic: 'ወደ ጸሎት ስፍራ መጣሁ', english: 'I Come to the Garden Alone' },
    author: { amharic: '', english: 'C Austin Miles' },
    verses: [
      { type: 'verse', amharic: 'ወደ ጸሎት ስፍራ መጣሁ መለድ ብዬ ጤዛው ሣይረግፍ\nከዚያ ስገባ ድምፅህን ሰማሁ ያምላክ ልጅ ሲመጣ አየሁ።', english: '' },
      { type: 'verse', amharic: 'ሲናገር የድምፁ ማማር አቆመው የዖፎችን መዝሙር\nበዚሁም ጊዜ ዜማን አሰማኝ ሁልጊዜ የማይረሳኝ።', english: '' },
      { type: 'verse', amharic: 'አብሬው እቀመጣለሁ ቀን አልፎ ጨለማ ቢሆንም\nፈተናን እንዳልፍ ይፈልገኛል በየዋህ ድምፅ ይጠራኛል።', english: '' },
      { type: 'chorus', amharic: 'ከኔ ጋር ሲጓዝ ሲያነጋግረኝ የራሱ እንደሆንሁ ሲነግረኝ\nአብረንም ሣለን ደስታ አገኘን ማንም ያላገኘውን።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '98',
    title: { amharic: 'ጌታ ሆይ ናፍቄሃለሁ', english: 'Blessed Lord How I Need Thee' },
    author: { amharic: '', english: 'F E Belden' },
    verses: [
      { type: 'verse', amharic: 'ጌታ ሆይ ናፍቄህአለሁ ደካማ ዕውር ሣለሁ\nኃይልና ማየት አገኘሁ ባንተ እታመናለሁ።', english: '' },
      { type: 'verse', amharic: 'የጽድቅህን ልብስ አልብሰኝ ኃጢዓት ስላቆሸሸኝ\nደካምነቴን አወቅሁኝ አምላካዊ ኃይል ስጠኝ።', english: '' },
      { type: 'verse', amharic: 'ብትመራኝ እድናለሁ ብለይህ እወድቃለሁ\nሁን ከኔ ጋር በችግሬ አንተ ሕይወት ብርሃኔ።', english: '' },
      { type: 'verse', amharic: 'ለወደፊት የሚመጣ ኃዘን ወይም ደስታ\nካንተ ሊለየኝ አይችልም የኔ ተስፋ አንተ ነህ።', english: '' },
      { type: 'chorus', amharic: 'በየሰዓት በየሰዐቱ መድኃኒቴን እሻዋለሁ።\nበየሰዓት በየሰዓቱ መድኃኒቴን እሻለሁ።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  {
    id: '99',
    title: { amharic: 'ጠንክሩ', english: 'Hold the Fort' },
    author: { amharic: '', english: 'P P Bliss' },
    verses: [
      { type: 'verse', amharic: 'ወንድሞቼ ተመልከቱ ባንዲራችንን\nኃይላችንም ተጨምርዋል ድል ልንነሳ ነን።', english: '' },
      { type: 'verse', amharic: 'ብርቱ ጠላት ተሰልፎ ሰይጣን ይመራል\nኃያል ሰዎች ወደቁብን ግን አንፈራም።', english: '' },
      { type: 'verse', amharic: 'ባለክብሩን ባንዲራ እዩ መለከት ስሙ\nበመሪያችን ስም ድል ነሣን ጠላትን ሁሉ።', english: '' },
      { type: 'verse', amharic: 'ጦርነቱ ቢበረታም እርዳታ ቀርቧል\nአዛዣችን አሁን መጥቶ አይዞአችሁ ይላል።', english: '' },
      { type: 'chorus', amharic: 'ጠንክሩ እየመጣሁ ነኝ ይላል ጌታችን\nእንሰራለን ጌታ የሱስ በፀጋህ እርዳን።', english: '' }
    ],
    tags: ['CHRISTIAN LIVING']
  },
  { 
    id: '100',
    title: { amharic: 'መንገዱን በደስታ አብሩ', english: 'Brighten the Way with a Smile' },
    author: { amharic: '', english: '' }, 
    verses: [], 
    tags: ['CHRISTIAN LIVING'] 
  }
];